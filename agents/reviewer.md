...
- `scope: blockers` (default) or `scope: full`;
...
`scope: blockers` → report Critical and Major only. `scope: full` → all six.
...
```json
{
  ...
  "findings": [
    {
      "severity": "Critical | Major | Minor | Question | Suggestion",
